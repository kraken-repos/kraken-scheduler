...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.IntegrationScenarios(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.IntegrationScenario, desired *v1alpha1.IntegrationScenario) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.SchedulerV1alpha1().IntegrationScenarios(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.SchedulerV1alpha1().IntegrationScenarios(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.IntegrationScenario) (*v1alpha1.IntegrationScenario, error) {
	...
	actual, err := r.Lister.IntegrationScenarios(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.SchedulerV1alpha1().IntegrationScenarios(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
