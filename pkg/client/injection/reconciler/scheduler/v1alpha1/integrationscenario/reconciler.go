...
import (
	...
	schedulerv1alpha1 "kraken.dev/kraken-scheduler/pkg/client/listers/scheduler/v1alpha1"
)

// Interface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.IntegrationScenario.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.IntegrationScenario. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.IntegrationScenario) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.IntegrationScenario.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.IntegrationScenario. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.IntegrationScenario) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.IntegrationScenario if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.IntegrationScenario.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.IntegrationScenario) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.IntegrationScenario if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.IntegrationScenario.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha1.IntegrationScenario) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.IntegrationScenario) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.IntegrationScenario resources.
type reconcilerImpl struct {
	...
	Lister schedulerv1alpha1.IntegrationScenarioLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister schedulerv1alpha1.IntegrationScenarioLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.IntegrationScenarios(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.IntegrationScenario, desired *v1alpha1.IntegrationScenario) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.SchedulerV1alpha1().IntegrationScenarios(desired.Namespace)
		...
		updater := r.Client.SchedulerV1alpha1().IntegrationScenarios(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.IntegrationScenario) (*v1alpha1.IntegrationScenario, error) {

	getter := r.Lister.IntegrationScenarios(resource.Namespace)
	...
	patcher := r.Client.SchedulerV1alpha1().IntegrationScenarios(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.IntegrationScenario) (*v1alpha1.IntegrationScenario, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.IntegrationScenario, reconcileEvent reconciler.Event) (*v1alpha1.IntegrationScenario, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
